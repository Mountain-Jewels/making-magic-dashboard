...
import { MonitorPlay, Sun, Camera, Shirt, Mic, Sparkles, RefreshCw, Send } from 'lucide-react'
...
export function StageView() {
  ...
  const [lightingPreset, setLightingPreset] = useState('warm_intimate')
  const [cameraPreset, setCameraPreset] = useState('static_closeup')
  ...
  async function handleSpeak() {
    ...
    setBusy(true)
    try {
      await metahumanSpeak('default', speakText, emotion)
  ...
  async function handleSetEmotion() {
    setBusy(true)
    try {
      await metahumanEmotion('default', emotion)
